use crate::datatype::{FollowCategory, FollowData, FollowError, DataKeys};
use crate::interface::FollowOperations;
use soroban_sdk::{symbol_short, Address, Env, Symbol, Vec, log};
...
impl<'a> FollowManager<'a> {
    ...
    pub fn add_follower(
        ...
        let mut product_followers: Vec<Address> = self.env
            .storage()
            .persistent()
        ...
        let mut followers: Vec<FollowData> = self.env
            .storage()
            .persistent()
            .get(&key)
            .unwrap_or_else(|| Vec::new(self.env));
        ...
        self.env.storage().persistent().set(&product_followers_key, &product_followers);

        // Add user to the AllUsers list if not already present
        let all_users_key = DataKeys::AllUsers;
        let mut all_users: Vec<Address> = self.env
            .storage()
            .persistent()
            .get(&all_users_key)
            .unwrap_or_else(|| Vec::new(self.env));
        ...
        if !all_users.contains(user) {
            ...
            self.env.storage().persistent().set(&all_users_key, &all_users);
        }
